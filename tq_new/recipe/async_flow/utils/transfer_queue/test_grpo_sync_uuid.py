...
"""GRPO synchronous training test for TransferQueue with UUID support.

This file simulates a GRPO (Group Relative Policy Optimization) training workflow
using the TransferQueue system with grouped sampling (get_n_samples=True) and UUID
for prompt identification.

Key differences from test_grpo_sync.py:
...
- Demonstrates UUID-based prompt tracking across sampling
...
def main():
    """Main function to run GRPO training simulation with UUID support."""
    logger.info("=" * 60)
    logger.info("GRPO Synchronous Training Simulation with UUID")
    logger.info("=" * 60)
    ...
    try:
        ...
        for round_num in range(1, NUM_ROUNDS + 1):
            ...
            # Each prompt is identified by a unique UUID
            pad_id = 0
            ...
            # This ensures all samples from the same prompt share the same UUID
            prompt_uuids = [f"uuid_r{round_num}_p{i}" for i in range(prompts_num)]
            prompt_uuids_list = []
            ...
            # Step 1: Put prompts with UUIDs to TransferQueue
            allocated_indexes = tq_client.put_experience(
                data_dict={
                    "prompt": padded_prompts,
                    "prompt_length": prompt_lengths_tensor,
                    "prompt_uuid": prompt_uuids_list,
                },
                unpad_pairs=[("prompt", "prompt_length", "right_pad")],
                topic=topic,
            )
            logger.info(f"Step 1 - Put prompts with UUIDs to indexes: {allocated_indexes}")
            ...
            logger.info(f"  Sampled UUIDs: {prompts_data['prompt_uuid']}")
            ...
            logger.info(f"  Training batch UUIDs: {grpo_experience['prompt_uuid']}")
        ...
        logger.info("-" * 60)
        logger.info("GRPO sync UUID test PASSED:")
        logger.info("  ✓ UUID-based prompt identification works correctly")
        ...
        logger.info("  ✓ Multi-round training works correctly with unique UUIDs")
