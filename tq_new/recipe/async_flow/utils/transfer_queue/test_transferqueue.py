...
def setup_transferqueue_manager():
    ...
    mgr = TransferQueueManager.options(num_cpus=1).remote(
...
def test_override_indexes_and_delete_experience_by_versions(topic_basic):
    ...
    pad_id = 0
    lengths = [3, 5, 4]
    # Use response and response_length (non-shared columns)
    ...
    assert remaining_count == 4  # Should have 4 remaining indexes (2,3,4,5)
...
def test_version_sampler_newest_mode(topic_basic):
    ...
    # First batch: indexes[0:n_samples] using version=0
    ...
    # Second batch: indexes[n_samples:2*n_samples] using version=1
    ...
    # Third batch: indexes[2*n_samples:] using version=2
...
def test_reset_all(topic_basic):
    ...
    try:
        ...
        # If we reach here, topic still exists (should fail)
        assert False, "Topic should not exist after reset_all"
    except Exception as e:
        # Expected - topic no longer exists
...
#     """
#     Scenario a: Write prompts in advance + batched responses, nspp = 4
#     Steps:
...
def test_scenario_b_multiple_complete_batches(topic_basic):
    """
    Scenario b: Multiple complete batch writes
...
def test_scenario_c_uuid_out_of_order_writes(topic_with_uuid):
    """
    Scenario c: UUID out-of-order writes
    ...
    n_samples = topic_with_uuid["n_samples"]
...
def test_scenario_d_incomplete_sequential_batches(topic_basic):
    """
    Scenario d: Incomplete sequential batch writes
...
#     """Shard-side test: Verify shared columns avoid duplicate storage through marking, reducing actual storage bytes."""
#     pad_id = 0
#     topic = topic_with_uuid["topic"]
#     tq_client = get_transferqueue_client()
#     tq_mgr = ray.get_actor("TransferQueueManager")
...
#     # Verify unique group count is {0, 1, 2}
#     # Actual group distribution: index 0-3 → group 0, index 4-5 → group 1, index 6 → group 1, index 8 → group 2
...
#     assert unique_groups == set({0, 1, 2}), f"Should have 3 unique groups, got {unique_groups}"

#     # ========== Verify shared column storage efficiency ==========
#     # Theoretical without sharing: If each index independently stores complete data, need to store sum of all batches
#     # i.e., 8 indexes' complete data = batch1 + batch2 + batch3 all shared column data
#     expected_without_sharing = (
#         # prompts: all 8 samples' prompt data
#         _calculate_tensor_bytes(prompts_batch1) +
#         _calculate_tensor_bytes(prompts_batch2) +
#         _calculate_tensor_bytes(prompts_batch3) +
#         # prompt_lengths: all 8 samples' prompt_length data
#         _calculate_tensor_bytes(prompt_lengths1) +
#         _calculate_tensor_bytes(prompt_lengths2) +
#         _calculate_tensor_bytes(prompt_lengths3) +
#         # prompt_uuid: all 8 samples' uuid data
...
#     # Verify shared column storage efficiency: compression ratio = actual storage / theoretical without sharing, should be < 1
#     compression_ratio = shared_bytes / expected_without_sharing
#     assert 0 < compression_ratio < 1, \
#         f"Compression ratio {compression_ratio:.2%} should be in (0, 1), indicating sharing mechanism effectively reduced storage"

#     # Verify all shared columns have data
#     assert storage_bytes.get("prompt", 0) > 0, "prompt column should have stored data"
#     assert storage_bytes.get("prompt_length", 0) > 0, "prompt_length column should have stored data"
#     assert storage_bytes.get("prompt_uuid", 0) > 0, "prompt_uuid column should have stored data"
...
#     """Calculate the byte size of pickle encoded data"""
#     total_bytes = 0
...
def test_bf16_serialization(topic_basic):
    """Test bf16 data type serialization and deserialization"""
    tq_client = get_transferqueue_client()
    topic = topic_basic["topic"]
    n_samples = topic_basic["n_samples"]
    ...
    # Verify returned tensor type is bf16
    assert data is not None
    assert len(idxs) == 6
...
def test_float8_serialization(topic_basic):
    """Test float8 data type serialization and deserialization"""
    tq_client = get_transferqueue_client()
    topic = topic_basic["topic"]
    n_samples = topic_basic["n_samples"]

    # Create float8 tensor
    pad_id = 0
    lengths = [3, 5, 4]
    ...
    # Verify returned tensor type is float8
    assert data is not None
    assert len(idxs) == 6
...
@pytest.fixture()
def topic_high_dim():
    """Create a topic for testing high-dimensional tensors"""
    tq_client = get_transferqueue_client()

    topic = "HIGH_DIM_TOPIC"
    prompts_num = 2
    n_samples = 2
    experience_columns = ["prompt", "prompt_length", "features_2d", "features_3d", "features_4d"]
    consumers = ["learner"]
...
@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_1d(topic_high_dim):
    """Test 1D tensor shape preservation"""
    tq_client = get_transferqueue_client()
    topic = topic_high_dim["topic"]
    n_samples = topic_high_dim["n_samples"]
    prompts_num = topic_high_dim["prompts_num"]
    ...
    # Create 1D feature tensors with different lengths
    features_1d = [torch.randn(l) for l in [10, 20, 15, 25]]
    ...
    allocated_indexes = tq_client.put_experience(
        data_dict={
            "prompt": padded_prompts,
            "prompt_length": prompt_lengths,
            "features_2d": features_1d,  # Using features_2d column for 1D test
        },
        topic=topic,
    )
    ...
    # Get and verify shape preservation
    data, idxs = tq_client.get_experience(
        consumer="learner",
        experience_columns=["features_2d"],
        experience_count=4,
        indexes=None,
        get_n_samples=False,
        topic=topic,
    ...
    assert data is not None
    assert len(idxs) == 4
    assert len(data["features_2d"]) == 4

    # Verify 1D shapes are preserved
    assert data["features_2d"][0].shape == (10,)
    assert data["features_2d"][1].shape == (20,)
    assert data["features_2d"][2].shape == (15,)
    assert data["features_2d"][3].shape == (25,)


@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_2d(topic_high_dim):
    ...
    # Create 2D feature tensors with different shapes
    features_2d = [torch.randn(i, j) for i, j in [(5, 10), (7, 12), (6, 8), (8, 15)]]
    ...
    # Verify 2D shapes are preserved
    assert data["features_2d"][0].shape == (5, 10)
    assert data["features_2d"][1].shape == (7, 12)
    assert data["features_2d"][2].shape == (6, 8)
    assert data["features_2d"][3].shape == (8, 15)


@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_3d(topic_high_dim):
    ...
    # Create 3D feature tensors with different shapes
    features_3d = [torch.randn(i, j, k) for i, j, k in [(2, 3, 4), (3, 5, 7), (2, 4, 6), (3, 4, 8)]]
    ...
    data, idxs = tq_client.get_experience(
        consumer="learner",
        experience_columns=["features_3d"],
        experience_count=4,
        indexes=None,
        get_n_samples=False,
        topic=topic,
    ...
    assert data is not None
    assert len(data["features_3d"]) == 4

    # Verify 3D shapes are preserved
    assert data["features_3d"][0].shape == (2, 3, 4)
    assert data["features_3d"][1].shape == (3, 5, 7)
    assert data["features_3d"][2].shape == (2, 4, 6)
    assert data["features_3d"][3].shape == (3, 4, 8)


@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_4d(topic_high_dim):
    """Test 4D tensor shape preservation"""
    tq_client = get_transferqueue_client()
    topic = topic_high_dim["topic"]
    ...
    # Create 4D feature tensors with different shapes (like CNN feature maps)
    features_4d = [torch.randn(i, j, k, l) for i, j, k, l in [(2, 3, 4, 5), (3, 4, 5, 6), (2, 4, 6, 7), (3, 5, 6, 8)]]
    ...
    data, idxs = tq_client.get_experience(
        consumer="learner",
        experience_columns=["features_4d"],
        experience_count=4,
        indexes=None,
        get_n_samples=False,
        topic=topic,
    ...
    assert data is not None
    assert len(data["features_4d"]) == 4

    # Verify 4D shapes are preserved
    assert data["features_4d"][0].shape == (2, 3, 4, 5)
    assert data["features_4d"][1].shape == (3, 4, 5, 6)
    assert data["features_4d"][2].shape == (2, 4, 6, 7)
    assert data["features_4d"][3].shape == (3, 5, 6, 8)


@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_mixed_dimensions(topic_high_dim):
    """Test mixed dimensional tensors in same put/get"""
    tq_client = get_transferqueue_client()
    topic = topic_high_dim["topic"]
    ...
    features_2d = [torch.randn(5, 10), torch.randn(7, 12), torch.randn(6, 8), torch.randn(8, 15)]
    features_3d = [torch.randn(2, 3, 4), torch.randn(3, 5, 7), torch.randn(2, 4, 6), torch.randn(3, 4, 8)]
    features_4d = [torch.randn(2, 3, 4, 5), torch.randn(3, 4, 5, 6), torch.randn(2, 4, 6, 7), torch.randn(3, 5, 6, 8)]

    # Put all dimensions together
    tq_client.put_experience(
        data_dict={
            "prompt": padded_prompts,
            "prompt_length": prompt_lengths,
    ...
    # Get and verify all dimensions
    data, idxs = tq_client.get_experience(
        consumer="learner",
        experience_columns=["features_2d", "features_3d", "features_4d"],
        experience_count=4,
        indexes=None,
        get_n_samples=False,
        topic=topic,
...
@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_data_integrity(topic_high_dim):
    """Test that data values are preserved with high-dimensional tensors"""
    tq_client = get_transferqueue_client()
    topic = topic_high_dim["topic"]
    ...
    # Create 3D tensors with known values
    features_3d = [
        torch.arange(24).reshape(2, 3, 4).float(),  # [2,3,4] with values 0-23
        torch.arange(60).reshape(3, 4, 5).float(),  # [3,4,5] with values 0-59
        torch.arange(30).reshape(2, 3, 5).float(),  # [2,3,5] with values 0-29
        torch.arange(56).reshape(4, 2, 7).float(),  # [4,2,7] with values 0-55
    ]
    ...
    torch.testing.assert_close(data["features_3d"][0], torch.arange(24).reshape(2, 3, 4).float())
    torch.testing.assert_close(data["features_3d"][1], torch.arange(60).reshape(3, 4, 5).float())
    torch.testing.assert_close(data["features_3d"][2], torch.arange(30).reshape(2, 3, 5).float())
    torch.testing.assert_close(data["features_3d"][3], torch.arange(56).reshape(4, 2, 7).float())


@pytest.mark.xdist_group(name="high_dim_test")
def test_high_dim_tensor_with_bf16(topic_high_dim):
    """Test high-dimensional tensors with bf16 dtype"""
    tq_client = get_transferqueue_client()
    topic = topic_high_dim["topic"]
    ...
    pad_id = 0
    lengths = [2, 3]
    prompts = _make_padded_prompts(lengths, pad_id=pad_id).to(torch.bfloat16)
    padded_prompts = prompts.repeat_interleave(2, dim=0)
    prompt_lengths = torch.tensor(lengths, dtype=torch.int32).repeat_interleave(2, dim=0).unsqueeze(1)

    # Create 2D bf16 tensors
    features_2d = [
        torch.randn(3, 5, dtype=torch.bfloat16),
        torch.randn(4, 6, dtype=torch.bfloat16),
        torch.randn(3, 7, dtype=torch.bfloat16),
    ...
    # Get and verify shape and dtype
    data, idxs = tq_client.get_experience(
        consumer="learner",
        experience_columns=["features_2d"],
        experience_count=4,
    ...
    # Verify shapes and bf16 dtype
    assert all(
        t.shape == expected_shape
        for t, expected_shape in zip(data["features_2d"], [(3, 5), (4, 6), (3, 7), (5, 8)], strict=False)
    )
    assert all(t.dtype == torch.bfloat16 for t in data["features_2d"])
