...
class Metric(ABC):
    ...
    def update(self, key="", value=None, cumulate=False):
        ...
        """
        self.logger.info("========= in metrics update start ==========")
        if cumulate:
            self.logger.info("cumulate1")
            if isinstance(value, dict):
                self.logger.info("========= in metrics, value is instance dict ==========")
                if cumulate:
                ...
                self.logger.info("========= value is not instance dict  ==========")
                if key in self.metric:
                    self.logger.info("========= key in metrics ==========")
                    if isinstance(self.metric[key], list):
                        self.metric[key].extend(value)
                    else:
                        self.metric[key] = value
                else:
                    self.logger.info("========= key not in metrics ==========")
                    self.metric[key] = [*value]
        else:
        ...
        self.logger.info("========= in metrics update finish ==========")
